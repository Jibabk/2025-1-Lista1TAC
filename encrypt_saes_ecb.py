...
def encrypt_saes_ecb(text, key):
    ...
    blocks = text_to_blocks(text)      # Converte o texto em binário e o divide em um bloco de 16 bits
    encrypted = []
    test = []
    for block in blocks:
    ...
    encrypted_bin = ''.join(encrypted)  # Converte o bloco cifrado em binário
    encrypted_hex = hex(int(encrypted_bin, 2))[2:]  # Converte o binário cifrado em hexadecimal
    encrypted_b64 = base64.b64encode(int(encrypted_bin, 2).to_bytes((len(encrypted_bin) + 7) // 8, 'big')).decode() # Converte o binário cifrado em base64
